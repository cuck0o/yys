...
Page({
  ...
  init: function () {
    ...
    for (var i = 0; i < tempList.length; i++) {
      ...
      if (app.globalData.userObj.card2.give.indexOf(tempList[i].id) != -1) {
        tempList[i].giveSelect = true;
        tempGiveArr.push(tempList[i]);
      }
      if (app.globalData.userObj.card2.need.indexOf(tempList[i].id) != -1) {
  ...
  onSubmit: function () {
    ...
    for (var i = 0; i < this.data.giveList.length; i++) {
      app.globalData.userObj.card2.give.push(this.data.giveList[i].id);
    }
    app.globalData.userObj.card2.need = [];
    for (var j = 0; j < this.data.needList.length; j++) {
      app.globalData.userObj.card2.need.push(this.data.needList[j].id);
    }
    // 保存
          ...
          for (var j = 0; j < res.data[k].card2.need.length; j++) {
            if (app.globalData.userObj.card2.give.indexOf(res.data[k].card2.need[j]) >= 0) {
              var obj = {
                ...
                "give": that.getNameByID(res.data[k].card2.need[j]),
